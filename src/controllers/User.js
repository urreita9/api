...
const createUser = async (req = request, res = response) => {
    ...
    if (user) {
        ...
        if (userCreated) {
            ...
            return res.status(500).json({ state: false, msg: 'Something went wrong' });
        }
    }
};
...
const beAdmin = async (req = request, res = response) => {
    ...
    if (user.role !== 'SUPER_ADMIN') {
        ...
        res.json({ state: false, msg: 'No puede cambiar el Rol de un SUPER_ADMIN' });
    }
};
...
const banUser = async (req = request, res = response) => {
    ...
    if (validUser.role === 'SUPER_ADMIN') {
        if (user.role === 'SUPER_ADMIN') {
            res.status(400).json({ state: false, msg: 'No puede aplicar ban a un SUPER_ADMIN' });
        } else {
...
const unBanUser = async (req = request, res = response) => {
    ...
    if (validUser.role === 'SUPER_ADMIN') {
        if (user.role === 'SUPER_ADMIN') {
            res.status(400).json({ state: false, msg: 'No puede aplicar unban a un SUPER_ADMIN' });
        } else {
